...
static Real Coeff_nu_0, Coeff_nubar_0,Coeff_nu_f,Coeff_nubar_f,t_L_0, t_L_1, dCoeff_nu,L_nu,L_nubar,L_nu_f,L_nubar_f,eps_nu,eps_nubar,eps_nu_f,eps_nubar_f,dCoeff_nubar,Omega_0, energy_floor, Mpns, Omega_dot, r_env1,r_env2,env_mass,env_vel;
  ...
  if (n==4) {
   ...
   std::stringstream msg;
    msg << "### FATAL ERROR in Mesh" << std::endl
        << "Argument different from 4 or 5 passed to Fermi function"<< std::endl;
    ATHENA_ERROR(msg);
    return 0.0;
  }
}
...
Real qdotQW(Real temp, Real x, Real ye, Real time, Real etaqw) {
  // temp is in MeV
  ...
  Real ferm5_0 = fermi_approx(5,0.0);
  Real cf = ye*(ferm5_eta/ferm5_0) + (1.0-ye)*(ferm5_negeta/ferm5_0);
  out -= 2.27*Na*cf*std::pow(temp,6.0); // cooling
  if(temp<Tc) {
...
if (pin->DoesParameterExist("hydro", "helm_temp_index")) {
      ...
      std::cout<<"Using Helm Temp passive scalars in PGEN\n";
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin) {
  ...
  if (COMP_DIV_B) {
    SetUserOutputVariableName(6, "divB");
  }    
  if (COMP_SRC && rot_flag) {
...
Real YeSource(Real temp, Real ye, Real x, Real time, Real Rho, Real etah) {
...
// Calculate Eta, send in T in K
  ...
  Real cc = 0.448*std::pow(temp,5.0)/f4_0;
  ...
  for (int k=pmb->ks; k<=pmb->ke; ++k) {
    for (int j=pmb->js; j<=pmb->je; ++j) {
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        Real Eta = pmb->peos->EtaFromRhoT(rho, temp, r_cell);
        temp = temp * 8.6173e-11;  //T in MeV
        Real qdot = qdotQW(temp, x, Ye, time, Eta); //MeV s^{-1} g^{-1} units

        Real de = dt*prim(IDN,k,j,i) * qdot * 1.6022e-6; 
        cons(IEN,k,j,i) += de;
        if (NSCALARS > 0 && ye_index >= 0) {
           Real Ye_i = prim_scalar(ye_index,k,j,i);
	   Real dYe = dt*YeSource(temp,Ye_i,x,time,rho,Eta);
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        ...
        if (NSCALARS > 0 && temp_index >= 0) {
           pmb->pscalars->r(temp_index,k,j,is-i) = pmb->pscalars->r(temp_index,k,j,is);
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) {
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      Real Eta = peos->EtaFromRhoT(phydro->w(IDN,k,j,i), temp, r_cell);
      temp = temp * 8.6173e-11;  //T in MeV
	  Real qdot = qdotQW(temp, x, Ye, pmy_mesh->time,Eta) * 1.6022e-6; // in ergs/s/g
...
#pragma omp simd
        for (int i=is; i<=ie; ++i) {
          user_out_var(6,k,j,i) = dflx(i)/vol(i);
        }
      }
    }
