...
//  Real T_g       = T_0;
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
          prim(IDN,k,j,is-i)                  = rho_0; //rho_new;
  ...
  //        Real Teq      = Arr[0];
  //        T_equilibrium = Teq;
          pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is); //Teq;
...
//          pmb->pscalars->r(t_index,k,j,is-i) = pmb->pscalars->r(t_index,k,j,is);
          ...
          if (NON_BAROTROPIC_EOS) {
            prim(IPR,k,j,is-i) = pmb->peos->PresFromRhoT(rho_new, T_g, r_scalar); //This one works for tau
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
        if (prim(IVX,k,j,ie) < 0.0) {
          ...
          // Switch to outflow BCs
          Real rgh           = pco->x1v(ie+i);
          Real re            = pco->x1v(ie);
          Real ratio         = re / rgh;
          prim(IVX,k,j,ie+i) = prim(IVX,k,j,ie); // Mdot * (1.989e33) / (4.0 * PI * SQR(rgh) * prim(IDN,k,j,ie+i));       //prim(IVX,k,j,ie);
          prim(IDN,k,j,ie+i) = prim(IDN,k,j,ie) * SQR(ratio); //Mdot * (1.989e33) / (4.0 * PI * SQR(rgh) * fabs(prim(IVX,k,j,ie+i))); //rho_f; //prim(IDN,k,j,ie) * SQR(ratio);//Mdot * (1.989e33) / (4.0 * PI * SQR(rgh) * fabs(prim(IVX,k,j,ie+i))); //
          prim(IVY,k,j,ie+i) = prim(IVY,k,j,ie) * ratio;
          prim(IVZ,k,j,ie+i) = 0.0; //prim(IVZ,k,j,ie) * ratio;
