...
static int IDT1, IDT2, IDT3, IDT4, IDT5, IDT6, IDT7, IDT8, IDT9, IDT10, IDT11, IDT12, IDT13;
...
// Final Ghozt zone values
Real P_GZ, V_GZ, Cs_GZ;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (use_IC_file) {
    ...
    AthenaArray<Real>& VGZ{ruser_mesh_data[9]};
    AthenaArray<Real>& CsGZ{ruser_mesh_data[9]};
...
void InflowOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                   FaceField &b, Real time, Real dt, int is, int ie, int js,
                   int je, int ks, int ke, int ngh) {
  AthenaArray<Real>& DATA1 = pmb->pmy_mesh->ruser_mesh_data[8];
  AthenaArray<Real>& DATA2 = pmb->pmy_mesh->ruser_mesh_data[9];
  AthenaArray<Real>& DATA3 = pmb->pmy_mesh->ruser_mesh_data[10];
  Real &P_Ghost{DATA1(0)};
  Real &V_Ghost{DATA2(0)};
  Real &Cs_Ghost{DATA3(0)};
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      Real theta = pco->x2v(j);
      for (int i=1; i<=ngh; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          V_GZ               = prim(IVX,ke,je,ie+1);
          Cs_GZ              = std::sqrt(pmb->peos->AsqFromRhoP(rho_f,prim(IPR,k,j,ie+i),r_scalar));
          P_Ghost            = P_GZ;
          V_Ghost            = V_GZ;
          Cs_Ghost           = Cs_GZ;
        }
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
     ...
     //   Real temp     = peos->TFromRhoP(phydro->w(IDN,k,j,i),phydro->w(IPR,k,j,i),efrac); // units of K
        ...
        AthenaArray<Real>& DATA1   = pmy_mesh->ruser_mesh_data[8];
        AthenaArray<Real>& DATA2   = pmy_mesh->ruser_mesh_data[9];
        AthenaArray<Real>& DATA3   = pmy_mesh->ruser_mesh_data[10];
        Real &Tequil{my_data(0)};
        Real &P_GhostZ{DATA1(0)};
        Real &V_GhostZ{DATA2(0)};
        Real &Cs_GhostZ{DATA3(0)};
