...
// Static Variables
// r=r_PNS equilibrium quantities: inner pressure, inner temperature, inner electron fraction, inner sound speed squared, GZ pressure modifier.
static Real T_eq, Ye_eq, dpdd_0, pModify;
...
//	Real* ye_ptr = pmb->pscalars->r(ye_index,k,j,ie+i);
     ...
     //     std::cout << "p_f (active zone)  = " << prim(IPR,k,j,ie) << " erg/cm^3" << std::endl;
          prim(IPR,k,j,ie+i) = p_f / pModify; // Constant Pressure
...
//        std::cout << "rho (u)  = " << phydro->u(IDN,k,j,i) << " g/cm^3" << std::endl;
        ...
        user_out_var(8,k,j,i)  = phydro->w(IPR,k,j,0);
