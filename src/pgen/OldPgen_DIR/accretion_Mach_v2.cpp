...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (pin->DoesParameterExist("problem", "MachNumber")) {
    Mach = pin->GetReal("problem","MachNumber");
    std::cout << "(pgen) using hand-set Mach number = " << Mach << std::endl;
  } else {
    Mach = -1
  }
...
//	Real* ye_ptr = pmb->pscalars->r(ye_index,k,j,ie+i);
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          if (Mach>0.0) {
            prim(IPR,k,j,ie+i) = rho_f * std::pow(v_f,2.0) / (gamma * std::pow(Mach,2.0)); // Constant Pressure
          } else {
            Real Cs    = std::sqrt(pmb->peos->AsqFromRhoP(rho_f,p_f,r_scalar));
            Real MachN = v_f / Cs
            prim(IPR,k,j,ie+i) = rho_f * std::pow(v_f,2.0) / (gamma * std::pow(MachN,2.0)); // Constant Pressure
          }
...
//        std::cout << "rho (u)  = " << phydro->u(IDN,k,j,i) << " g/cm^3" << std::endl;
        ...
        user_out_var(8,k,j,i)  = p_f / pModify; // outer ghost zone
