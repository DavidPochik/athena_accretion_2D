      ...
      for (int i=1; i<=ngh; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) {
         ...
         // Real lightspeed    = 2.99792458e10;              // Speed of light in cm/s
          prim(IPR,k,j,ie+i) = pmb->peos->PresFromRhoT(rho_f,pmb->pscalars->r(t_index,k,j,ie+i),r_scalar); //rho_f * (etot + std::pow(lightspeed,2.0)) / ((gamma * std::pow(Mach * lightspeed, 2.0))/(std::pow(v_f, 2.0)) - 1.0);
